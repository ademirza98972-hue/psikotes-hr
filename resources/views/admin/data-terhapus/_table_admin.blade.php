<div>
    <div class="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden">
        <div class="mb-2 px-6 pt-4 text-[12px] text-on-surface-variant">
            Menampilkan <strong>{{ $items->firstItem() ?? $items->total() }}</strong> dari <strong>{{ $items->total() }}</strong> akun admin/staff
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead style="position: sticky; top: 0; z-index: 5;">
                    <tr style="background: #0f2230; border-bottom: 1px solid #0a1a25;">
                        <th class="px-6 py-4 text-[11px] font-bold uppercase tracking-wider" style="color: #7db8c2;">Nama</th>
                        <th class="px-6 py-4 text-[11px] font-bold uppercase tracking-wider" style="color: #7db8c2;">Email</th>
                        <th class="px-6 py-4 text-[11px] font-bold uppercase tracking-wider" style="color: #7db8c2;">No HP</th>
                        <th class="px-6 py-4 text-[11px] font-bold uppercase tracking-wider" style="color: #7db8c2;">Peran</th>
                ...
                <tbody class="divide-y divide-outline-variant/30">
                    @forelse ($items as $item)
                        <tr class="hover:bg-surface-container/30 transition-colors">
                            ...
                            <td class="px-6 py-5 text-body-md text-on-surface-variant">{{ $item->peran?->nama_peran ?? '-' }}</td>
                            <td class="px-6 py-5">
                            ...
                            <td class="px-6 py-5 text-right">
                                <div class="flex items-center justify-end gap-3">
                                    <form action="{{ route('admin.data-terhapus.pulihkan', ['jenis' => 'admin', 'id' => $item->id]) }}" method="POST" class="inline">
                                        @csrf
                                    ...
                                    <form action="{{ route('admin.data-terhapus.hapus-permanen', ['jenis' => 'admin', 'id' => $item->id]) }}" method="POST" class="inline"
                                          onsubmit="return confirm('Peringatan: Tindakan ini akan menghapus data akun admin/staff secara permanen dan tidak dapat dipulihkan. Lanjutkan?')">
                                        @csrf
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-10 text-center text-body-md text-on-surface-variant">Tidak ada akun admin/staff yang dihapus.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
