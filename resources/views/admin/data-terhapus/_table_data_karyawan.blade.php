            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($items as $item)
                    <tr class="hover:bg-slate-50">
                        <td class="px-4 py-3 font-mono text-slate-700">{{ $item->nik_karyawan }}</td>
                        <td class="px-4 py-3 font-medium text-slate-900">{{ $item->nama_karyawan }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ $item->departemen?->nama_departemen ?? '-' }}</td>
                        <td class="px-4 py-3 text-slate-600">{{ ucfirst($item->status) }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-4 py-10 text-center text-sm text-slate-500">Tidak ada data karyawan yang dihapus.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
