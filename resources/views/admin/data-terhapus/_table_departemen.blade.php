<div>
    <div class="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden">
        <div class="mb-2 px-6 pt-4 text-[12px] text-on-surface-variant">
            Menampilkan <strong>{{ $items->firstItem() ?? $items->total() }}</strong> dari <strong>{{ $items->total() }}</strong> departemen
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-surface-container-low border-b border-outline-variant">
                        <th class="px-6 py-4 text-[11px] font-bold text-on-surface-variant uppercase tracking-wider">Nama Departemen</th>
                        <th class="px-6 py-4 text-[11px] font-bold text-on-surface-variant uppercase tracking-wider">Jumlah Posisi Terkait</th>
                ...
                <tbody class="divide-y divide-outline-variant/30">
                        ...
                        <tr class="hover:bg-surface-container/30 transition-colors">
                            <td class="px-6 py-5">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="font-semibold text-primary">{{ $item->nama_departemen }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-5">
                                <div class="inline-flex items-center px-2 py-1 rounded bg-secondary-container/30 border border-secondary-container text-on-secondary-container">
                                    <span class="text-label-sm font-bold">{{ $item->posisi_count }} posisi</span>
                                </div>
                            </td>
                            <td class="px-6 py-5">
                            ...
                            <td class="px-6 py-5 text-right">
                                <div class="flex items-center justify-end gap-3">
                                    <form action="{{ route('admin.data-terhapus.pulihkan', ['jenis' => 'departemen', 'id' => $item->id]) }}" method="POST" class="inline">
                                        @csrf
                                    ...
                                    <form action="{{ route('admin.data-terhapus.hapus-permanen', ['jenis' => 'departemen', 'id' => $item->id]) }}" method="POST" class="inline"
                                          onsubmit="return confirm('Peringatan: Tindakan ini akan menghapus data departemen secara permanen dan tidak dapat dipulihkan. Lanjutkan?')">
                                        @csrf
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-10 text-center text-body-md text-on-surface-variant">Tidak ada departemen yang dihapus.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
