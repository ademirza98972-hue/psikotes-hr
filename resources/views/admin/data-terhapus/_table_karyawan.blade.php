<div>
    <div class="bg-surface-container-lowest border border-outline-variant rounded-xl overflow-hidden">
        <div class="mb-2 px-6 pt-4 text-[12px] text-on-surface-variant">
            Menampilkan <strong>{{ $items->firstItem() ?? $items->total() }}</strong> dari <strong>{{ $items->total() }}</strong> akun karyawan
        </div>
        <div style="overflow: auto; max-height: calc(100vh - 300px);">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-outline-variant/30">
                        ...
                        <tr class="hover:bg-surface-container/30 transition-colors">
                            ...
                            <td class="px-6 py-5 text-right">
                                <div class="flex items-center justify-end gap-3">
                                    <form action="{{ route('admin.data-terhapus.pulihkan', ['jenis' => 'karyawan', 'id' => $item->id]) }}" method="POST" class="inline">
                                        @csrf
                                    ...
                                    <form action="{{ route('admin.data-terhapus.hapus-permanen', ['jenis' => 'karyawan', 'id' => $item->id]) }}" method="POST" class="inline"
                                          onsubmit="return confirm('Peringatan: Tindakan ini akan menghapus data akun karyawan secara permanen dan tidak dapat dipulihkan. Lanjutkan?')">
                                        @csrf
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center text-body-md text-on-surface-variant">Tidak ada akun karyawan yang dihapus.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
