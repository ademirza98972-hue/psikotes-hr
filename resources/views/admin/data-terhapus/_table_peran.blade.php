            ...
            <tbody class="divide-y divide-slate-100">
                @forelse ($items as $item)
                    <tr class="hover:bg-slate-50">
                        <td class="px-4 py-3 font-medium text-slate-900">{{ $item->nama_peran }}</td>
                        <td class="px-4 py-3 text-slate-600 max-w-[200px] truncate" title="{{ $item->deskripsi }}">{{ $item->deskripsi ?? '-' }}</td>
                        <td class="px-4 py-3 text-center text-slate-600">{{ $item->pengguna_count }}</td>
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-4 py-10 text-center text-sm text-slate-500">Tidak ada peran yang dihapus.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
