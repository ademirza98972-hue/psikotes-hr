...
<div class="w-full rounded-lg border border-slate-200 bg-white p-6 shadow-sm"
    ...
    <div class="mb-4 flex items-center justify-between">
        <h2 class="text-lg font-semibold text-slate-900">Tambah Peran Baru</h2>
    ...
    <form method="POST" action="{{ route('admin.peran.simpan') }}" class="space-y-4">
        @csrf
        ...
        <div class="grid gap-4 md:grid-cols-2">
            ...
            <div>
                ...
                <textarea id="deskripsi" name="deskripsi" rows="2" maxlength="1000"
                          class="mt-1 block w-full rounded-md border border-slate-300 px-3 py-2 text-sm shadow-sm focus:border-[#2C5F6F] focus:outline-none focus:ring-1 focus:ring-[#2C5F6F]">{{ old('deskripsi') }}</textarea>
            </div>
        </div>
                    ...
                    <div class="rounded-md border border-slate-200 bg-slate-50 p-3">
                        ...
                        <div id="{{ $prefixId }}" class="grid grid-cols-1 gap-1.5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                            @foreach ($daftar as $izin)
                                <label class="flex items-center gap-2 text-sm font-medium uppercase text-slate-700">
