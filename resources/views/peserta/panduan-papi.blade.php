@extends('layouts.peserta', ['judulHalaman' => 'Panduan PAPI Kostick'])
...
if ($adaTimerTotal && $adaTimerPerSoal) {
    $infoWaktu = $alatTes->durasi_total_menit . ' menit total · ' . $alatTes->batas_waktu_per_soal_detik . ' detik per soal';
    $ikonWaktu = 'schedule';
...
$ruleWaktu = ($adaTimerTotal || $adaTimerPerSoal)
    ? 'Perhatikan <strong>batas waktu</strong> yang tertera di layar — timer berjalan sejak soal pertama dimulai.'
...
<div class="max-w-2xl mx-auto pb-8 space-y-5">
    ...
    <div>
        <div class="inline-flex items-center gap-2 rounded-lg px-3 py-1.5 mb-3"
            ...
            <span class="text-[11px] font-bold uppercase tracking-wider">PAPI Kostick — Kepribadian Kerja</span>
        </div>
        <h2 class="text-[22px] font-semibold text-[#00303c]">Personality and Preference Inventory</h2>
    ...
    <div class="rounded-xl border border-[#e0e3e5] bg-white shadow-sm">
        ...
        <div class="px-6 py-5 space-y-3">
            <p class="text-[14px] text-[#191c1e] leading-relaxed">
                Setiap soal menyajikan <strong>dua pernyataan</strong> tentang perilaku atau preferensi dalam situasi kerja. Pilih <strong>satu pernyataan</strong> yang <strong>paling menggambarkan diri Anda</strong> saat bekerja.
            </p>
            <div class="rounded-lg px-4 py-3 flex items-start gap-2"
                 style="background:#f0fdf4;border:1px solid #bbf7d0">
                <span class="material-symbols-outlined text-[16px] shrink-0 mt-0.5" style="color:#15803d">lightbulb</span>
                <p class="text-[13px]" style="color:#15803d">
                    Tidak ada jawaban benar atau salah. Jawab berdasarkan <strong>bagaimana Anda benar-benar berperilaku di tempat kerja</strong>, bukan bagaimana Anda ingin terlihat.
                </p>
            </div>
        </div>
    </div>

    {{-- Perbedaan dengan EPPS --}}
    <div class="rounded-xl border border-[#e0e3e5] bg-white shadow-sm">
        <div class="border-b border-[#e0e3e5] px-6 py-4">
        ...
        <div class="divide-y divide-[#f0f2f3]">
            @php
            $rules = [
                'Fokus pada <strong>perilaku Anda di lingkungan kerja</strong> — bukan kehidupan sehari-hari secara umum.',
                'Jawab secara <strong>jujur dan spontan</strong> — pilih yang paling mendekati kebiasaan Anda bekerja.',
                'Setiap soal <strong>harus dijawab</strong>, tidak boleh dilewati.',
                $ruleWaktu,
            ];
            @endphp
            ...
            <div class="flex items-start gap-4 px-6 py-4">
                <div class="shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-[11px] font-bold text-white mt-0.5"
                     style="background:#15803d;min-width:1.5rem">{{ $i + 1 }}</div>
    ...
    <div class="rounded-xl border border-[#e0e3e5] bg-white shadow-sm">
        ...
        <div class="px-6 py-5">
            <p class="text-[12px] text-[#71787c] mb-4">Pilih <strong>salah satu</strong> pernyataan yang paling menggambarkan Anda saat bekerja:</p>
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 mb-4">
                <div class="flex items-start gap-3 rounded-xl p-4 border-2 border-[#e0e3e5]">
                    ...
                    <p class="text-[13px] text-[#40484b] leading-relaxed">Saya senang memimpin dan mengarahkan rekan kerja.</p>
                </div>
                <div class="flex items-start gap-3 rounded-xl p-4 border-2"
                    ...
                    <div class="shrink-0 mt-0.5 h-4 w-4 rounded-full border-2 flex items-center justify-center"
                         style="border-color:#15803d;background:#15803d;min-width:1rem">
                        <div class="h-1.5 w-1.5 rounded-full bg-white"></div>
                    </div>
                    <p class="text-[13px] leading-relaxed font-medium" style="color:#166534">Saya lebih suka bekerja mengikuti arahan yang sudah ditetapkan.</p>
                </div>
            </div>
            <div class="rounded-lg bg-[#f7f9fb] border border-[#e0e3e5] px-4 py-3 flex items-start gap-2">
                ...
                <p class="text-[12px] text-[#40484b] leading-relaxed">Pernyataan kedua dipilih (ditandai hijau). Pilih sesuai yang <strong>paling menggambarkan perilaku kerja Anda</strong> sehari-hari.</p>
            </div>
        </div>
    </div>

    {{-- Tombol Mulai --}}
    <form method="POST" action="{{ route('peserta.tes.panduan-papi-mulai', $sesiId) }}">
        @csrf
        <button type="submit"
        ...
        <p class="mt-2 text-center text-[11px] text-[#71787c]">
            Pastikan Anda siap — timer akan berjalan setelah tombol ini ditekan
        </p>
    </form>
